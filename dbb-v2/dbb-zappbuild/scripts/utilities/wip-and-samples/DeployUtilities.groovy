...
deployApplicationPackage (props.workDir )  

def deployApplicationPackage (String workDir) {	
	/* pre- expanded 
	...
	*/
	...
	println "** Parse the pdsMapping file"	
	def pdsMapping = new File("$workDir/pdsMapping.yaml").text 
	pdsMap = [:]
	pdsMapping.split("\n").each { line ->
	...
	def resourcesBlock = { unit, member  ->
		...
		copy.file(new File("$workDir/${unit.folder}/${member.name}")).member("${member.name}").copy()
	}

	yamlUtils.parseDeploymentUnitsAndResources("$workDir/app.yaml", deploymentUnitsBlock, resourcesBlock)
	
	return "${workDir}/app.yaml"
}
